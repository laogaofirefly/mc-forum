...
(() => {
 ...
 async function render(){if(!currentWorld||!currentMap)return;loading.classList.remove('hidden');loading.textContent='正在加载地图瓦片…';layer.innerHTML='';const w=currentWorld.name||currentWorld.id,m=currentMap.prefix||currentMap.name||'flat';try{const r=await fetch(`${tileBase}s-manifest/${safe(w)}/${safe(m)}?_=${Date.now()}`,{cache:'no-store'}),p=await r.json(),tiles=p.tiles||[];if(!tiles.length)throw Error('未找到已渲染的地图瓦片');const size=128;tiles.forEach((file,index)=>{const match=file.match(/(-?\d+)_(-?\d+)\.png$/),x=match?Number(match[1]):(index%20)-10,y=match?Number(match[2]):Math.floor(index/20)-10,img=new Image();img.className='dyn-tile';img.style.left=(x*size)+'px';img.style.top=(y*size)+'px';img.src=`${tileFileUrl}?world=${safe(w)}&map=${safe(m)}&path=${safe(file)}`;img.onerror=()=>img.remove();layer.appendChild(img)});apply();loading.classList.add('hidden');hint.textContent=`${currentWorld.title||currentWorld.name} · ${currentMap.title||currentMap.name||'默认图层'} · ${tiles.length} 块瓦片`}catch(e){loading.textContent=e.message;hint.textContent='地图瓦片加载失败'}}
