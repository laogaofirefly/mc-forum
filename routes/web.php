...
// ── MCSM API 路由（替换 / 增强直接 RCON 方式）──

// MCSM 配置信息
Route::get('/admin/console/mcsm-info', function () {
    if (! auth()->check() || ! auth()->user()->isAdmin()) {
        return response()->json(['ok' => false, 'message' => '仅管理员可访问'], 403);
    }
    $mcsm = new \App\Services\McsmService();
    return response()->json(array_merge(['ok' => true], $mcsm->configInfo()));
})->name('admin.console.mcsm-info')->middleware('auth');

// MCSM 状态（覆盖原有 /admin/console/status）
Route::get('/admin/console/status', function () {
    if (! auth()->check() || ! auth()->user()->isAdmin()) {
        return response()->json(['ok' => false, 'message' => '仅管理员可访问'], 403);
    }
    ...
    // 优先使用 MCSM
    $mcsm = new \App\Services\McsmService();
    if ($mcsm->isConfigured()) {
        try {
            $status = $mcsm->getStatus();
            $status['source'] = 'mcsm';
            return response()->json($status);
        } catch (\Throwable $e) {
            return response()->json(['ok' => false, 'message' => 'MCSM 状态获取失败：' . $e->getMessage()], 500);
        }
    }
    ...
    if (! $running) {
        $socket = @fsockopen($mcHost, $mcPort, $errno, $errstr, 2);
        if ($socket) { fclose($socket); $running = true; $method = 'port'; }
    }
    if (! $running) {
        if (PHP_OS_FAMILY === 'Windows') {
            @exec('tasklist /FI "IMAGENAME eq javaw.exe" 2>&1', $output, $code);
    ...
    return response()->json([
        'ok' => true, 'running' => $running, 'method' => $method, 'source' => 'direct',
    ]);
})->name('admin.console.status')->middleware('auth');

// MCSM 日志（覆盖原有 /admin/console/log）
Route::get('/admin/console/log', function (\Illuminate\Http\Request $request) {
    if (! auth()->check() || ! auth()->user()->isAdmin()) {
        return response()->json(['ok' => false, 'message' => '仅管理员可访问'], 403);
    }
    ...
    if ($mcsm->isConfigured()) {
        try {
            $after = (int) $request->input('after', 0);
            $limit = min(500, max(10, (int) $request->input('lines', 200)));
            $result = $mcsm->getLog($after, $limit);
            $result['source'] = 'mcsm';
            return response()->json($result);
        } catch (\Throwable $e) {
            return response()->json(['ok' => false, 'message' => 'MCSM 日志获取失败：' . $e->getMessage()], 500);
        }
    }
    ...
    if ($after >= $fileSize) {
        return response()->json(['ok' => true, 'lines' => [], 'size' => $fileSize, 'pos' => $fileSize, 'source' => 'direct']);
    }
    ...
    if ($after === 0 && $fileSize > 0) {
        $startPos = max(0, $fileSize - 131072);
        fseek($handle, $startPos);
        if ($startPos > 0) fgets($handle);
    } else {
        fseek($handle, $after);
    }
    ...
    while (($line = fgets($handle, 8192)) !== false) {
        ...
        $lines[] = ['n' => $lineNum, 'raw' => $raw, 'chat' => $chat];
        if (count($lines) >= $limit) break;
    }
    fclose($handle);

    return response()->json([
        'ok' => true, 'lines' => $lines, 'size' => $fileSize,
        'pos' => $pos, 'log_path' => $logPath, 'source' => 'direct',
    ]);
})->name('admin.console.log')->middleware('auth');

// MCSM 命令执行（增强 /admin/rcon）
Route::post('/admin/rcon', function (\Illuminate\Http\Request $request) {
    if (! auth()->check() || ! auth()->user()->isAdmin()) {
        return response()->json(['ok' => false, 'message' => '仅管理员可执行命令'], 403);
    }

    $request->validate(['command' => 'required|string|min:1|max:200']);
    ...
    if ($mcsm->isConfigured()) {
        try {
            $result = $mcsm->sendCommand($command);
            $result['source'] = 'mcsm';
            $result['time'] = now()->format('H:i:s');
            $result['user'] = auth()->user()->name;
            return response()->json($result);
        } catch (\Throwable $e) {
            return response()->json(['ok' => false, 'message' => 'MCSM 命令执行失败：' . $e->getMessage()], 500);
        }
    }
    ...
    try {
        ...
        return response()->json([
            ...
            'time' => now()->format('H:i:s'), 'user' => auth()->user()->name, 'source' => 'rcon',
        ]);
    } catch (\Throwable $e) {
        return response()->json(['ok' => false, 'message' => '执行失败：' . $e->getMessage()], 500);
    }
...
Route::post('/admin/console/start', function () {
    ...
    if ($mcsm->isConfigured()) {
        try {
            ...
            return response()->json(['ok' => false, 'message' => 'MCSM 启动失败：' . $e->getMessage()], 500);
        }
    }
    ...
    try {
        ...
        proc_close($process);
        return response()->json([
            'ok' => true, 'message' => '启动命令已发送，请等待 10-30 秒后刷新状态',
            'command' => $command, 'cwd' => $cwd, 'source' => 'direct',
        ]);
    } catch (\Throwable $e) {
        return response()->json(['ok' => false, 'message' => '启动失败：' . $e->getMessage()], 500);
    }
...
// MCSM 停止
Route::post('/admin/console/stop', function () {
    if (! auth()->check() || ! auth()->user()->isAdmin()) {
        return response()->json(['ok' => false, 'message' => '仅管理员可操作'], 403);
    }
    $mcsm = new \App\Services\McsmService();
    if (! $mcsm->isConfigured()) {
        return response()->json(['ok' => false, 'message' => 'MCSM 未配置，停止仅支持通过 MCSM 面板'], 500);
    }
    try {
        $result = $mcsm->stop();
        $result['source'] = 'mcsm';
        ...
        return response()->json(['ok' => false, 'message' => 'MCSM 停止失败：' . $e->getMessage()], 500);
    }
})->name('admin.console.stop')->middleware('auth');

// MCSM 重启
Route::post('/admin/console/restart', function () {
    if (! auth()->check() || ! auth()->user()->isAdmin()) {
        return response()->json(['ok' => false, 'message' => '仅管理员可操作'], 403);
    }
    $mcsm = new \App\Services\McsmService();
    if (! $mcsm->isConfigured()) {
        return response()->json(['ok' => false, 'message' => 'MCSM 未配置，重启仅支持通过 MCSM 面板'], 500);
    }
    try {
        $result = $mcsm->restart();
        $result['source'] = 'mcsm';
        ...
        return response()->json(['ok' => false, 'message' => 'MCSM 重启失败：' . $e->getMessage()], 500);
    }
})->name('admin.console.restart')->middleware('auth');
